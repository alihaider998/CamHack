...
CONTAINER="camhack"
IMAGE="alihaider998/camhack:latest"
IMG_MIRROR="ghcr.io/alihaider998/camhack:latest"
...
if [[ ! $check_container == $CONTAINER ]]; then
	...
	docker create \
		--interactive --tty \
		--volume ${MOUNT_LOCATION}:/camhack/auth/ \
		--network host \
		--name "${CONTAINER}" \
		"${IMAGE}"
fi
...
# docker run --rm -ti --network="host" -v ${MOUNT_LOCATION}:/camhack/auth/ --name "$CONTAINER" "$IMAGE"
